...
import { StoreState, useAppDispatch } from "@/app/store/store";

const ProductDetails = () => {
    const dispatch = useAppDispatch();
    const params = useParams();
    const router = useRouter();
    const product = useSelector((state: StoreState) => state.productReducer.product)
    ...
    useEffect(() => {
        ...
        if (!product || product.id !== productId) {
            dispatch(fetchProductDetails(productId.toString()));
        }
    }, [params.id]);
